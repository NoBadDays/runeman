...
import nobaddays.runman.domain.Product;
import nobaddays.runman.domain.Company;
import nobaddays.runman.domain.Measure;
import nobaddays.runman.repository.ProductRepository;
...
public class ProductResourceIT {
    ...
    public static Product createEntity(EntityManager em) {
        ...
        // Add required entity
        Measure measure;
        if (TestUtil.findAll(em, Measure.class).isEmpty()) {
            measure = MeasureResourceIT.createEntity(em);
            em.persist(measure);
            em.flush();
        } else {
            measure = TestUtil.findAll(em, Measure.class).get(0);
        }
        product.setMeasureId(measure);
        return product;
    }
    /**
    ...
    public static Product createUpdatedEntity(EntityManager em) {
        ...
        Measure measure;
        if (TestUtil.findAll(em, Measure.class).isEmpty()) {
            measure = MeasureResourceIT.createUpdatedEntity(em);
            em.persist(measure);
            em.flush();
        } else {
            measure = TestUtil.findAll(em, Measure.class).get(0);
